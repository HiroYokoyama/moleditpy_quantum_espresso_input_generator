...
Some files in `qe_input_generator/` are **vendored copies** owned by
...
python scripts/sync_shared.py ../moleditpy_quantum_espresso_input_generator      # writes the copy + the manifest
```
...
```bash
cd ../moleditpy-periodic-shared
python scripts/sync_shared.py ../moleditpy_quantum_espresso_input_generator --check   # drifted? non-zero exit
```
