...
`moleditpy_quantum_espresso_input_generator` — a MoleditPy plugin that writes
`pw.x` input from the molecule open in the editor, a CIF file, or the CIF Viewer
panel. Entry point `qe_input_generator/__init__.py`, dialog in `main_dialog.py`,
all text generation in `writer.py`.

## Shared modules — read this before editing

Some files in `qe_input_generator/` are **not owned by this repository**. A
...
|---|---|---|---|
| `cell_model.py` | `periodic-cell-model` | 0.7.0 | VASP, CP2K, Slab Builder |
| `elements.py` | `periodic-elements` | 0.1.0 | VASP, CP2K, Slab Builder |
| `cell_preview.py` | `periodic-cell-preview` | 0.1.0 | VASP, CP2K, Slab Builder |
| `structure_panel.py` | `periodic-structure-panel` | 0.5.0 | VASP, CP2K |
